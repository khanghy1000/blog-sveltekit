<script lang="ts">
  ...
  const pageCount = Math.ceil(data.postCount / 10);

  let urlPage;
  let currentPage: number;
  $: {
    urlPage = $page.url.searchParams.get('page');
    currentPage = urlPage && +urlPage > 0 ? +urlPage : 1;
  }
</script>
...
<div class="max-w-2xl mx-auto p-5">
  {#each data?.posts as post}
    <div>
      <a href={`/posts/${post.id}`} class="text-2xl font-semibold">{post.title}</a>
      <p class="mt-5">{formatDate(post.createdAt)}</p>
      <div class="divider" />
    </div>
  {/each}
  ...
  <div class="pagination justify-center">
    ...
    <!-- {#each Array.from({ length: pageCount }) as page, i}
      <a class="btn" class:btn-active={currentPage === i + 1} href={`/?page=${i + 1}`}>{i + 1}</a>
    {/each} -->
    ...
    {#if currentPage >= 1 && currentPage <= pageCount}
      <btn class="btn btn-active" href={`/?page=currentPage`}>{currentPage}</btn>
    {/if}
    ...
    {#if currentPage != pageCount}
      <a href={`/?page=${pageCount}`} class="btn">{pageCount}</a>
    {/if}
  ...
  <form method="get" action="/" class="flex flex-row items-center gap-5 max-w-xs mx-auto mt-5">
    <span>Page</span>
    <input type="text" placeholder="page" class="input" name="page" />
